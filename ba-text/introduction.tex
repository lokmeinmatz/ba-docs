...
\begin{figure}
  \begin{tikzpicture}[node distance=2cm]
    ...
    \node(2-label)[right of=layout,anchor=west,xshift=1cm]{Lean UX};
    \draw [arrow] (bl) -- (tech1);
    ...
    \node (prod) [round, below of=feedback] {Production deployment};
    \draw [arrow] (deploy) -- (prod);
    \draw [arrow] (prod) -- (feedback);
  
    \begin{scope}[on background layer]
      \node[draw,dashed,gray,rounded corners,fill=black!10,fit=(dug) (denv) (bl)]{};
      \node(1-label)[right of=dug,anchor=west,xshift=3cm]{Design Thinking};
      \node[draw,dashed,gray,rounded corners,fill=black!10,fit=(tech1) (layout) (proto) (2-label)]{};
      \node[draw,dashed,gray,rounded corners,fill=black!10,fit=(deploy) (prod) (feedback)]{};
      \node(3-label)[left of=prod,anchor=east,xshift=-3cm]{Agile};
      \end{scope}
  \end{tikzpicture}
  \caption{Development Process for the UI Editor}
	\label{fig:process}
\end{figure}
